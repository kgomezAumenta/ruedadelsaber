...
export async function POST(request: NextRequest) {
    try {
        const { nombre, pais_id, logo_url, banner_url } = await request.json();

        const [result] = await pool.query<ResultSetHeader>(
            'INSERT INTO marcas_bayer (nombre, pais_id, logo_url, banner_url) VALUES (?, ?, ?, ?)',
            [nombre, pais_id, logo_url || null, banner_url || null]
        );

        return NextResponse.json({ id: result.insertId, nombre, pais_id, logo_url, banner_url }, { status: 201 });
    } catch (error) {
...
export async function PUT(request: NextRequest) {
    try {
        const { id, nombre, pais_id, logo_url, banner_url } = await request.json();

        await pool.query(
            'UPDATE marcas_bayer SET nombre = ?, pais_id = ?, logo_url = ?, banner_url = ? WHERE id = ?',
            [nombre, pais_id, logo_url || null, banner_url || null, id]
        );

        return NextResponse.json({ id, nombre, pais_id, logo_url, banner_url });
    } catch (error) {
